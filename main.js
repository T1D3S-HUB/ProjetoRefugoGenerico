...
function gerarTabelaRelatorioTela() {
    ...
    if (apontamentosFiltrados.length === 0) {
     ...
     // 1. AGRUPAMENTO E SOMA POR LOTE (Cód. Interno + Data Fund.)
     apontamentosFiltrados.forEach(ap => {
         ...
         if (!gruposDePecas[chaveGrupo]) {
             gruposDePecas[chaveGrupo] = {
                 ...
                 apontamentosDoGrupo: [],
                 totalLiberadasLote: 0, // Acumulador local
                 totalRefugadasLote: 0  // Acumulador local
             };
         }
         gruposDePecas[chaveGrupo].apontamentosDoGrupo.push(ap);

         if (ap.tipo === 'liberada') {
             // SOMA LIBERADAS DO LOTE
             gruposDePecas[chaveGrupo].totalLiberadasLote += ap.qtd;
         } else if (ap.tipo === 'refugo') {
             // SOMA REFUGADAS DO LOTE (Pode haver múltiplos apontamentos de refugo por cavidade)
     ...
     chavesOrdenadas.forEach(chaveGrupo => {
         ...
         // CÁLCULO IMPLÍCITO E ROBUSTO (LOCAL AO LOTE)
         const totalVerificadoLote = grupo.totalLiberadasLote + grupo.totalRefugadasLote;
         const producaoPorCavidade = numCavidades > 0 ? Math.floor(totalVerificadoLote / numCavidades) : 0; 
         
         // 2. PRIMEIRA PASSAGEM: DISTRIBUIÇÃO E IDENTIFICAÇÃO DO REMANEJAMENTO
         ...
         const refugosPorCavidadeMapa = {}; // Usado para a segunda passagem
         ...
         for (let cavNum = cavInicial; cavNum <= cavFinal; cavNum++) {
             ...
             // Max Liberadas Posível (Respeitando o limite de produção implícito)
             const maxLiberadasPossivel = producaoPorCavidade - qtdRefugadaTotalCav;

             const qtdLiberadaInicial = liberadasIniciais[cavNum] || 0;

             if (qtdLiberadaInicial > maxLiberadasPossivel) {
                 // Excesso: Esta cavidade recebeu mais do que poderia.
                 const excesso = qtdLiberadaInicial - maxLiberadasPossivel;
                 remanejamento += excesso;
                 
                 // Aloca o máximo possível (que pode ser 0)
                 liberadasFinais[cavNum] = Math.max(0, maxLiberadasPossivel); 
             } else {
         ...
         // 3. SEGUNDA PASSAGEM: REDISTRIBUIÇÃO DO REMANEJAMENTO (Para manter a soma total)
         if (remanejamento > 0 && cavidadesAptas.length > 0) {
             ...
             let index = 0;
             while (remanejamento > 0) {
                 const cavNum = aptasComRefugo[index % aptasComRefugo.length].cavNum;
                 
                 // A redistribuição deve respeitar o limite MAX_POSSÍVEL, mas como a primeira passagem
                 // já tirou o excesso do limite (e remanejou), esta distribuição é apenas para
                 // fechar a conta do Total Liberado INPUT. 
                 
                 // Se o total remanescente for grande, a distribuição igualitária é a mais justa.
                 // Usaremos distribuição simples para garantir que a soma total seja mantida (43).
         ...
         for (let cavNum = cavInicial; cavNum <= cavFinal; cavNum++) {
             ...
             const qtdRefugadaTotalCav = refugosParaEstaCavidadeData.reduce((total, refugo) => total + (refugo.quantidade || 1), 0);
             ...
             // USAR O VALOR FINAL CALCULADO APÓS A REDISTRIBUIÇÃO
